...
impl<E: Pairing, QAP: R1CSToQAP<E::ScalarField, GeneralEvaluationDomain<E::ScalarField>>> Groth16<E, QAP> {
        ...
        C: ConstraintSynthesizer<E::ScalarField>,
    {
        ...
        let h_query = FixedBase::msm::<E::G1>(
            ...
            g1_window,
            &g1_table,
            &QAP::h_query_scalars(m_raw - 1, t, zt, delta_inverse)?,
        );
...
impl Groth16 {
    ...
    pub fn generate_random_parameters_with_reduction<C>(
        circuit: C,
        rng: &mut impl Rng,
    ) -> R1CSResult<ProvingKey<ark_bn254::Bn254>>
    where
        C: ConstraintSynthesizer<ark_bn254::Fr>,
    {
        let alpha = ark_bn254::Fr::rand(rng);
        let beta = ark_bn254::Fr::rand(rng);
        let gamma = ark_bn254::Fr::rand(rng);
        let delta = ark_bn254::Fr::rand(rng);

        let g1_generator = ark_bn254::G1Projective::rand(rng);
        let g2_generator = ark_bn254::G2Projective::rand(rng);
    ...
    pub fn generate_parameters_with_qap<C>(
        ...
        gamma: ark_bn254::Fr,
        delta: ark_bn254::Fr,
        g1_generator: ark_bn254::G1Projective,
        g2_generator: ark_bn254::G2Projective,
        rng: &mut impl Rng,
    ) -> R1CSResult<ProvingKey<ark_bn254::Bn254>>
    where
        C: ConstraintSynthesizer<ark_bn254::Fr>,
    {
        ...
        let num_instance_variables = cs.num_instance_variables();
        let (a, b, c, zt, qap_num_variables, m_raw) =
            gpu::GpuLibsnarkReduction::instance_map_with_evaluation(cs, &t)?;
        end_timer!(reduction_time);
        ...
        let scalar_bits = ark_bn254::Fr::MODULUS_BIT_SIZE as usize;
        ...
        // Compute the H-query
        let h_time = start_timer!(|| "Calculate H");
        let h_query = FixedBase::msm::<ark_bn254::G1Projective>(
            scalar_bits,
            g1_window,
            &g1_table,
        ...
        let vk = VerifyingKey::<ark_bn254::Bn254> {
            ...
            gamma_g2: gamma_g2.into_affine(),
            delta_g2: delta_g2.into_affine(),
            gamma_abc_g1: ark_bn254::G1Projective::normalize_batch(&gamma_abc_g1),
        };

        let batch_normalization_time = start_timer!(|| "Convert proving key elements to affine");
        let a_query = ark_bn254::G1Projective::normalize_batch(&a_query);
        let b_g1_query = ark_bn254::G1Projective::normalize_batch(&b_g1_query);
        let b_g2_query = ark_bn254::G2Projective::normalize_batch(&b_g2_query);
        let h_query = ark_bn254::G1Projective::normalize_batch(&h_query);
        let l_query = ark_bn254::G1Projective::normalize_batch(&l_query);
        end_timer!(batch_normalization_time);
        end_timer!(setup_time);
